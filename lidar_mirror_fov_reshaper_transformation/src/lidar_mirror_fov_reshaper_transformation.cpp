...
std::vector<double> lidarMirrorFOVReshaperTF::getNormalizedVector(const std::vector<double> &vec_src)
{
...
std::vector<double> lidarMirrorFOVReshaperTF::_getVectorToTransformedDataPoint(
    ...
    double mag_distance_vec)
{
  ...
  for (size_t i = 0; i < unit_reflection_vec.size(); i++)
  { // r
    if (diff_mag == 0)
    {
      RCLCPP_ERROR(
...

{
  ...
  for (size_t i = 0; i < unit_reflection_vectors.size(); i++)
  {
    ...
    std::vector<double> reflection_point_to_tf_data_point =
        lidarMirrorFOVReshaperTF::_getVectorToTransformedDataPoint(
    ...
    const std::vector<double> &helper_p3, std::vector<double> &normal_vec)
{
  ...
  // ensure normal vec dimensions are correct
  if (normal_vec.size() != helper_p1.size())
  {
    normal_vec.resize(helper_p1.size(), 0.0);
  };

  std::vector<double> u_vec = {helper_p2[0], helper_p2[1], helper_p2[2]};
  std::vector<double> v_vec = {helper_p3[0], helper_p3[1], helper_p3[2]};
    ...
    pcl::PointCloud<pcl::PointXYZI> *transformed_cloud_right_mirror)
{
  ...
  for (const auto &point : src_cloud_front.points)
  ...
  lidarMirrorFOVReshaperTF::normalizePoints(src_cloud_left_mirror, normalized_distance_vectors_left_mirror);
  ...
  lidarMirrorFOVReshaperTF::getPointMagnitude(src_cloud_left_mirror, mag_distance_vectors_left_mirror);
  lidarMirrorFOVReshaperTF::getPointMagnitude(src_cloud_right_mirror, mag_distance_vectors_right_mirror);
    ...
    double tgt_area_angle_max, double angle_increment /*= 1 */)
{
  ...
  return tgt_angle + tgt_area_angle_min;
};